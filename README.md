...
* ブロック一覧パネルのふるまい
  * 通常のmakecode環境では使える全てのブロックが一覧のパネルに出現しますが、tutorialでは(学習者が探すのが難しくなるため)必要な(というよりtutorialで言及した)パーツのみが現れます
  * markdown中のコードに現れた関数などが自動的にパネルに出現するようです
  * 明示的にパネルに出現させることもできるらしい
  * 上のnullなコードによる処理の破綻は、この振る舞いがおかしくなる
